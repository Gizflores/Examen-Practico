...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBAction func ActivadorBoton(_ sender: Any) {
        if (b1 && b2 && b3 == true) {
            ...
            let alert = UIAlertController(title: "Success", message: "Contraseña Correcta", preferredStyle: UIAlertControllerStyle.alert)
            alert.addAction(UIAlertAction(title: "Click", style: UIAlertActionStyle.default, handler: nil))
            self.present(alert, animated: true, completion: nil)
        }
        if (b1 || b2 || b3 == false) {
            let alert = UIAlertController(title: "Fail", message: "La contraseña es incorrecta", preferredStyle: UIAlertControllerStyle.alert)
            alert.addAction(UIAlertAction(title: "Click", style: UIAlertActionStyle.default, handler: nil))
            self.present(alert, animated: true, completion: nil)
        }
